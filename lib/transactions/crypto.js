...
function getTransactionBytes (transaction) {
	...
	function isSendTransaction () {
		if (transaction.asset && transaction.asset.note && transaction.asset.note != "") {
		  var note = transaction.asset.note;
		} else {
		  var note = "";
		}

		return {
			assetBytes: Buffer.from(note),
			assetSize: Buffer.from(note).length
		};
	}
...
function createTransactionBuffer (transaction, options) {
	...
	function assignTransactionBuffer (transactionBuffer, assetSize, assetBytes) {
		...
		if (transaction.recipientId) {
			var recipient = transaction.recipientId.slice(0, -3);
...
function getAddress (publicKey) {
	...
	var address = bignum.fromBuffer(temp).toString() + 'LWF';
	return address;
}
